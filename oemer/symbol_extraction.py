...
def parse_clefs_keys(clefs_keys, unit_size, clef_size_ratio=3.5, max_clef_tp_ratio=0.45):
    ...
    clef_label = pred_symbols(clef_box, MODEL_NAME) # model_name
    key_label = pred_symbols(key_box, MODEL_NAME) # model_name
...
def parse_rests(line_box, unit_size):
    ...
    for box in bboxes:
        ...
        pred = predict(region, MODEL_NAME) # model_name
        """
        if "8th" in pred:
            pred = predict(region, "rests_above8") # model_name
        valid_box.append(box)
        label.append(pred)
        """
